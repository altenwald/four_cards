defmodule FourCards.EventManager do
  use GenStage

  defp via(name) do
    {:via, Registry, {FourCards.EventManager.Registry, name}}
  end

  def exists?(name) do
    case Registry.lookup(FourCards.EventManager.Registry, name) do
      [{_pid, nil}] -> true
      [] -> false
    end
  end
  ...
  def get_pid(name) do
    [{pid, nil}] = Registry.lookup(FourCards.EventManager.Registry, name)
    pid
  end
